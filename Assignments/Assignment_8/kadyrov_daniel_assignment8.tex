...
\begin{solution}
    ...
    % If the users are technically savvy, I would let them submit, query, and see their log entries through an developer API. If they require a GUI to do these tasks, I would build a simple form where they can add additional fields. In the same GUI, I will have a search bar to query log entries and a profile type page to see their log entries. Although security is not mentioned in the problem statement, I would assume that the users will have a login to access the application.\\ 
